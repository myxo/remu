...
use crate::{
    ...
    state::{FrontendCommand, MsgOperation},
};
...
fn process_event(
    ...
    match update.content {
        UpdateContent::Message(message) => {
            ...
            if msg_text.starts_with("/start") {
                // very special case
                let res = engine.add_user(
                    UserInfo {
                        uid: user.id as i64,
                        name: user.username.as_ref().unwrap(),
                        chat_id: message.chat.id,
                        first_name: &user.first_name,
                        last_name: user.last_name.as_ref().unwrap(),
                        tz: -3,
                    },
                    message.message_id,
                );
...
fn handle_command_to_frontend(
    ...
    for cmd in cmds {
        ...
        match cmd {
            state::FrontendCommand::send { text } => {
                front.send_message(uid, &text, None)?;
            }
            state::FrontendCommand::calendar(at_calendar_command) => {
                ...
                match at_calendar_command.msg_op {
                    MsgOperation::Edit { id } => front.edit_message(uid, id, msg, keyboard)?,
                    MsgOperation::DeleteAndSendNew { old_id } => {
                        front.delete_message(uid, old_id)?;
                        front.send_message(uid, msg, keyboard)?;
                    }
                    MsgOperation::Sendnew => front.send_message(uid, msg, keyboard)?,
                }
            }
            state::FrontendCommand::keyboard { action_type, text } => match action_type {
                state::KeyboardType::Main => {
                    front.send_message(uid, &text, Some(make_main_action_keyboard()))?;
                }
                state::KeyboardType::Hour => {
                    front.send_message(uid, &text, Some(make_hour_keyboard()))?;
                }
                state::KeyboardType::Minute => {
                    front.send_message(uid, &text, Some(make_minute_keyboard()))?;
                }
            },
